...
export class SearchlocationPage {
  ...
  Postcode:any;
  address: any;
  constructor( private toastCtrl:ToastController, private nativeGeocoder: NativeGeocoder, private viewCtrl:ViewController,  public actionSheetCtrl: ActionSheetController,  public platform: Platform,  private alertCtrl: AlertController,  public loadingCtrl: LoadingController, public geolocation: Geolocation,  public storage: Storage,  public zone: NgZone,public navCtrl: NavController, public navParams: NavParams) {
  ...
  return new Observable((sub: any) => {
    google.maps.event.addListener(autocomplete, 'place_changed', () => {
      ...
      if (!place.geometry) {
        ...
        sub.next(place.geometry.location);
      // this.viewCtrl.dismiss({lat:this.lat ,lng :this.lng,postcode:this.Postcode});
       // this.navCtrl.push(SearchpagePage,{lat:this.lat ,lng :this.lng,postcode:this.Postcode})
        //sub.complete();
      }
    });
  });
    ...
    this.geolocation.getCurrentPosition(locationOptions).then(
      (position) => {
        this.loading.dismiss().then(() => {
          ...
          this.nativeGeocoder.reverseGeocode(position.coords.latitude,  position.coords.longitude, options1)
...
addInfoWindow(marker, content) {
  ...
  done( )
  {
    ...
    {
      console.log(this.address,'in address condtion')
      this.viewCtrl.dismiss({lat:this.lat ,lng :this.lng,postcode:this.address});
    }
   else
   {
    console.log(this.Postcode,'in Postcode condtion')
    this.viewCtrl.dismiss({lat:this.lat ,lng :this.lng,postcode:this.Postcode});
   }
