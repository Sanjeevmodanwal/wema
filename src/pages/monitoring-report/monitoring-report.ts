...
/**
 * Generated class for the MonitoringReportPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-monitoring-report',
  templateUrl: 'monitoring-report.html',
})
export class MonitoringReportPage {
  ...
  ionViewDidEnter() {
    ...
    this.getMonitoringReport(this.filter);
  }

  /**
   * Get member visit reports
   */
  async getMonitoringReport(filter: any) {
    ...
    if (response != null && response.hasOwnProperty('records') && response['records'].length > 0){
      this.monitoringReport = response['records'][0];
